        ...
        <div class="row my-3">
            <?php if(session()->get('isUserLoggedIn')): ?>
                <div class="col-2 col-md-2 offset-3">
                    <a href="<?php echo base_url('logout') ?>"><button id="logoutBtn" class="btn btn-secondary">Kijelentkezés</button></a>
                </div>
            <?php else: ?>
                <div class="col-2 col-md-2 offset-3">
                    <a href="<?php echo base_url('login') ?>"><button id="loginBtn" class="btn btn-secondary">Bejelentkezés</button></a>
                </div>
            <?php endif; ?>
                ...
                if (res.username != undefined && res.username != null) {
                    $.each(res, function () {
                        $('#posts').append(
                            ...
                            '</div>'+
                            '</div>'+
                            '<div class="col-10 col-md-8 d-flex justify-content-center pt-3'+ res.color +'" id="blog-post">'+
                            '<p class="blog-post-text p-3 full-width">'+ res.posttext +'</p>'+
                            '</div>'
                        )
                    })
                }
                else {
                    console.log('minden jó! c:')
                }
            },
            error: function (res) {
                console.log('te kurvaaaaaaaaaaaaaaaaaaaaaa');
            }
        });
    }, 1000);
